...
class RegisterTransform extends Transform {
    ...
    static final String EXT_NAME = RegisterPlugin.EXT_NAME
    Project project
    ...
    RegisterTransform(Project project) {
        ...
        println("RegisterTransform.1------------------------------------------------------------start\n" +
                "currentProject: ${project}\n" +
                "RegisterTransform.1------------------------------------------------------------end\n")
    }
    ...
    void transform(Context context, Collection<TransformInput> inputs
        ...
        println("RegisterTransform.transform.2------------------------------------------------------------start\n" +
                "extension.size: ${extension.list.size()}\n" +
                "RegisterTransform.transform.2------------------------------------------------------------end\n")

        extension.list.each { ext ->
            println("RegisterTransform.transform.3------------------------------------------------------------start\n" +
                    "ext.fileContainsInitClass: ${ext.fileContainsInitClass}\n" +
            ...
            if (ext.fileContainsInitClass) {
                ...
                println("RegisterTransform.transform.4------------------------------------------------------------start\n" +
                        ...
                        "ext.interfaceName: ${ext.interfaceName}\n" +
                        "RegisterTransform.transform.4------------------------------------------------------------end\n")
                if (ext.classList.isEmpty()) {
                    project.logger.error("No class implements found for interface:" + ext.interfaceName)
                } else {
                    ext.classList.each {
                        println("RegisterTransform.transform.5------------------------------------------------------------start\n" +
                                "it: ${it}\n" +
                                "RegisterTransform.transform.5------------------------------------------------------------end\n")
                    }
    ...
    private static void scanClassFile(File file, String root, boolean leftSlash, CodeScanner scanProcessor, File dest) {
        ...
        println("RegisterTransform.scanClassFile.1------------------------------------------------------------start\n" +
                "root: ${root}\n" +
                "file: ${file}\n" +
                "path: ${path}\n" +
                "dest: ${dest}\n" +
                "RegisterTransform.scanClassFile.1------------------------------------------------------------end\n")
        if (file.isFile()) {
            def entryName = path
            if (!leftSlash) {
            ...
            if (scanProcessor.shouldProcessClass(entryName)) {
                println("RegisterTransform.scanClassFile.2------------------------------------------------------------start\n" +
                        "root: ${root}\n" +
                        "file: ${file}\n" +
                        "path: ${path}\n" +
                        "dest: ${dest}\n" +
                        "entryName: ${entryName}\n" +
                        "RegisterTransform.scanClassFile.2------------------------------------------------------------end\n")
